...
Its custom `RegripCore` namespace, `.resi` public interface,
`rescript-vitest` compiler dependency, and `.res.mjs` compiler suffix
deliberately mirror `packages/core`, so this canary can isolate
compiler-layout assumptions before they reach the published package.
...
```text
Time.res
  → Time.res.mjs         ReScript runtime (rescript build)
...
1. ReScript writes `src/Time.res.mjs`.
...
4. The raw ReScript runtime (`Time.res.mjs`) is copied beside the compiled wrapper in `dist/src/` — tsc does not do this for non-TS files, and the compiled wrapper's own relative import needs it there to resolve.
...
The CI job is deliberately non-blocking while this toolchain capability is under investigation. The canary now matches core's namespace, public-interface, dependency, and `.res.mjs` settings; its green result means the next isolated step is a one-module `packages/core` spike using `@genType` on the `.resi` declaration, rather than changing core-wide output or exports.
