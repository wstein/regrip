...
let faceIndex = (face: string): int => switch face {
| "U" => 0
| "R" => 1
| "F" => 2
| "D" => 3
...
let normal = (face: string): vector => switch face {
| "U" => {x: 0, y: 1, z: 0}
| "R" => {x: 1, y: 0, z: 0}
| "F" => {x: 0, y: 0, z: 1}
| "D" => {x: 0, y: -1, z: 0}
...
let applyRegrip = (frame: t, notationToken: string): unit => {
  ...
  frame.logicalToPhysical = positions
    ->Array.map(index => {
...
let translate = (frame: t, move: string): string => {
  if String.length(move) == 0 {
    ...
    let logicalIndex = positions->Array.find(index => charAt(frame.logicalToPhysical, index) == physicalFace)
    switch logicalIndex {
    | Some(index) => `${charAt(faceOrder, index)}${String.substring(move, ~start=1, ~end=String.length(move))}`
    | None => move
    }
  }
}
...
let orientation = (frame: t): orientation => {
  ...
  {right: normal(rightFace), up: normal(upFace), front: normal(frontFace), rightFace, upFace, frontFace}
}
